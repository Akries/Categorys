...
- (UIImage *)setupQRImg:(NSString *)url Size:(CGFloat)size {
    ...
    [filter setValue:@"Q" forKey:@"inputCorrectionLevel"];//识别率,设置纠错等级越高；即识别越容易，值可设置为L(Low) |  M(Medium) | Q | H(High)
    // 4.获取输出的二维码
    CIImage *outputImage = [filter outputImage];
    // 5.将CIImage转换成UIImage，并放大显示
    UIImage *qrImg = [self createNonInterpolatedUIImageFormCIImage:outputImage withSize:size];
    return qrImg;
}
...
- (UIImage *)createRoundedRectImage:(UIImage *)image withSize:(CGSize)size withRadius:(NSInteger)radius {
    ...
    [self addRoundedRectToPath:contextRef Rect:rect Width:radius Height:radius];
...
- (void)addRoundedRectToPath:(CGContextRef)contextRef Rect:(CGRect)rect Width:(float)widthOfRadius Height:(float)heightOfRadius {
    float fw, fh;
